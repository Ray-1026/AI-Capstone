...
from CoDeF.utils.image_utils import parse_image_size
from CoDeF.utils.image_utils import load_image
from CoDeF.utils.image_utils import resize_image
from CoDeF.utils.image_utils import list_images_from_dir
...
def create_video(dataname):
    from glob import glob
    import cv2

    video_visualizer = VideoVisualizer(
        path=f"../DDcolor_output/{dataname}/output.mp4", frame_size=None, fps=25.0
    )
    img_folder = f"../DDcolor_output/{dataname}"
...
if __name__ == "__main__":
    ...
    video_visualizer = VideoVisualizer(path="new/output.mp4", frame_size=None, fps=25.0)
    img_folder = "new/"
    imgs = sorted(glob(img_folder + "/*.png"))
    for img in imgs:
        image = cv2.imread(img)
